...
function DashboardContent() {
  ...
  const [currentConversationId, setCurrentConversationId] = useState<string>(() => {
    // Generate initial proper UUID v4
  ...
  // Function to delete conversation from Supabase
  const deleteConversation = async (convId: string) => {
    ...
    try {
      ...
      setConversationTitles(prev => {
        ...
        delete newTitles[convId]
        return newTitles
      })

      // If deleting current conversation, switch to new one
  ...
  const startNewConversation = (persona: string = 'generic') => {
    ...
    setCurrentConversationId(newId)
    setMessages([])
    setCurrentPersona(persona as any)

    // Focus on input when starting new chat
    setTimeout(() => {
  ...
  // Function to change persona (ALWAYS creates new conversation)
  const changePersona = (newPersona: string) => {
    if (currentPersona !== newPersona) {
      // Always start a new conversation when changing persona
  ...
  // Clean up old localStorage data on mount (remove legacy data)
  useEffect(() => {
    // Remove all old localStorage data to prevent conflicts
    try {
      localStorage.removeItem('miky-conversations')
      localStorage.removeItem('miky-conversation-titles')
      localStorage.removeItem('conversationHistory')
      localStorage.removeItem('conversationTitles')
    } catch (error) {
      ...
      try {
        ...
        if (conversations && conversations.length > 0) {
          ...
          conversations.forEach(conv => {
            ...
            history[conv.id] = messages.map(msg => ({
              ...msg,
          ...
          // Load most recent conversation if we have conversations
          if (conversations.length > 0) {
            const mostRecent = conversations[0]
            setCurrentConversationId(mostRecent.id)
            setMessages(history[mostRecent.id] || [])
            setCurrentPersona(mostRecent.persona as any || 'generic')
          }
        }
      } catch (error) {
        console.error('Error loading conversations:', error)
  ...
  useEffect(() => {
    const saveConversationToSupabase = async () => {
      if (!user || !currentConversationId || messages.length === 0) return

      try {
        const title = conversationTitles[currentConversationId] ||
                      generateChatTitle(messages.find(m => m.role === 'user')?.content || 'New Chat')
    ...
    // Debounce saves
    const timeoutId = setTimeout(saveConversationToSupabase, 1000)
    return () => clearTimeout(timeoutId)
  }, [messages, currentConversationId, conversationTitles, user, currentPersona])
  ...
  useEffect(() => {
    ...
    if (!chatLoading && messageInputRef.current) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Improved focus retention - use requestAnimationFrame and multiple attempts
      const refocusInput = () => {
        requestAnimationFrame(() => {
          if (messageInputRef.current && document.activeElement !== messageInputRef.current) {
            messageInputRef.current.focus()
      ...
      // Try multiple times to ensure focus is maintained
      refocusInput()
  ...
  const confirmDeleteConversation = () => {
    if (conversationToDelete) {
      // Use the deleteConversation function which handles everything
